...
//============================================================================== 
// 2010-07-22
//Modifications made by Robert Garofalo to allow use of pre-generated thumbnails
...
(function($){
	$.fn.extend({ 
		infiniteCarousel: function(options)
		{
			var defaults = 
			{
				// special variable for using pre-generated thumbnails
				// if THUMBPREFIX = 'none' then standard thumbnail generation occurs
				// otherwise set the var to file prefix used on image thumbnails 
        		// EX: '_thmb'
				THUMBPREFIX: 'thmb_',
				transitionSpeed: 800,
				displayTime: 6000,
				...
				easeLeft: 'linear',
				easeRight: 'linear',
				imagePath: 'scripts/infinite-carousel/images/',
				inView: 1,
				padding: '0px',
				advance: 1,
				showControls: true,
		...
		var options = $.extend(defaults, options);
			...
			//textholder background, height, and opacity set here
			$('#textholder'+randID+'_'+i).height(imgHeight*o.textholderHeight).css({});
			//removed from inner brackets above, will set from page CSS file {'backgroundColor':'#DDD','opacity':'0.5'}
			...
			if(o.displayThumbnails)
			{
				...
				$(obj).after('<div id="thumbs'+randID+'" style="position:relative;overflow:auto;clear:left;text-align:left;margin:auto;padding-top:5px;"></div>');

//MODS HERE (Robert G - 07-22-10)=================================================================================
				var thumb, imgUrl;
				var thumbPrefix = o.THUMBPREFIX;
					//
					//alert(thumbPrefix);
					//if (thumbPrefix != 'none') alert("Custom thumbnails in use!");
				for(i=0; i<=numImages-1; i++)  //for(i=0; i<=numImages-1; i++)
				{
					if (i == numImages-1) { imgUrl = ""; } //$('img:eq('+(i)+')', obj).attr('src');}
					else {
					imgUrl = $('img:eq('+(i+1)+')', obj).attr('src');
					}
						//alert("counter: " + i + "\n" + "imgUrl: " + imgUrl);
					if (thumbPrefix == 'none') {
						thumb = imgUrl; 
						...
						if (imgUrl != "") {
						var urlPieces = imgUrl.split("/");
						var urlEnd = urlPieces.pop();
						
							//alert("ORIGINAL URL: " + imgUrl);
							//alert("urlEnd BEFORE:  " + urlEnd);
						
						urlEnd = thumbPrefix + urlEnd;
							//alert("urlEnd AFTER:  " + urlEnd);
						
						urlPieces.push(urlEnd);
						thumb = urlPieces.join("/");
							//alert("NEW URL: " + thumb);
						}
						else {
							//alert("Skipping routine, imgUrl is blank");
							thumb = imgUrl;
						}
					}
//RG Mod: removed a few styles here to offload them to the carousel stylesheet===================================================
// Also added: 
// 1. some additional generic classes with the 'thumb' style for page individualization
// 2. named anchor for having browser bring picture window into view, when clicking elements in very long thumb lists;
// should make this a variable later, allowing multiple instances of Carousel
					$('#thumbs'+randID).append('<a href="#Gallery0"><div class="thumb s1 s2" id="thumb'+randID+'_'+(i+1)+'" style="cursor:pointer;background-image:url('+thumb+');display:inline;float:left;width:'+o.thumbnailWidth+';height:'+o.thumbnailHeight+';line-height:'+o.thumbnailHeight+';padding:0;overflow:hidden;text-align:center;border:2px solid #ccc;font-size:'+o.thumbnailFontSize+';color:#000;text-shadow:0 0 3px #fff">'+(i+1)+'</div></a>');
...
//MODS HERE (Robert G - 07-22-10)===========================================================================================
				...
				imgUrl = $('img:first', obj).attr('src');
					//alert("Final thumbnail: " + imgUrl);
				if (thumbPrefix == 'none') {
					thumb = imgUrl; 
				}
				else {					
